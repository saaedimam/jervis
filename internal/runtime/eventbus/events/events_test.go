...
func TestValidatePriority(t *testing.T) {
	if err := events.ValidatePriority(0); err != nil {
		t.Fatalf("expected priority 0 to be valid: %v", err)
	}
	if err := events.ValidatePriority(-100); err != nil {
		t.Fatalf("expected priority -100 to be valid: %v", err)
	}
	if err := events.ValidatePriority(100); err != nil {
		t.Fatalf("expected priority 100 to be valid: %v", err)
	}
...
func TestEventBuilderSuccessfulBuild(t *testing.T) {
	...
	if evt.Priority() != 50 {
		t.Errorf("expected Priority 50, got %d", evt.Priority())
	}
...
func TestValidateEventValidationFailures(t *testing.T) {
	...
	// Missing ID
	builder := events.NewBuilder().SetType("runtime.test.event").SetSource("test")
	...
	// Invalid Type in Builder
	builderInvalidType := events.NewBuilder().SetID(evtID).SetType("INVALID_TYPE").SetSource("test")
	...
	// Empty Source
	builderSource := events.NewBuilder().SetID(evtID).SetType("runtime.test.event").SetSource("")
	...
	builderPriority := events.NewBuilder().SetID(evtID).SetType("runtime.test.event").SetSource("test").SetPriority(200)
	...
	// Empty Version
	builderVersion := events.NewBuilder().SetID(evtID).SetType("runtime.test.event").SetSource("test").SetVersion("")
...
func TestUninitializedBuilderAndNilMetadata(t *testing.T) {
	// Directly construct Builder struct without NewBuilder() to test nil metadata check
	uninitBuilder := &events.Builder{}
	uninitBuilder.SetMetadata("k", "v").SetVersion("1.0.0")

	evtID, _ := types.NewEventID("evt-uninit")
	uninitBuilder.SetID(evtID).SetType("runtime.test.event").SetSource("test")
	...
	// Test nil metadata in envelope directly
	emptyEnvelope := &mockEventNilMetadata{id: evtID}
	if err := events.ValidateEvent(emptyEnvelope); err != nil {
		t.Fatalf("unexpected validation error: %v", err)
	}
}

type mockEventNilMetadata struct {
	id types.EventID
}

func (m *mockEventNilMetadata) ID() types.EventID           { return m.id }
func (m *mockEventNilMetadata) Type() string                { return "runtime.test.event" }
func (m *mockEventNilMetadata) Source() string              { return "test" }
func (m *mockEventNilMetadata) Timestamp() types.Timestamp  { return types.Now() }
func (m *mockEventNilMetadata) CorrelationID() string       { return "corr" }
func (m *mockEventNilMetadata) CausationID() string         { return "cause" }
func (m *mockEventNilMetadata) Priority() int               { return 0 }
func (m *mockEventNilMetadata) Payload() any                { return nil }
func (m *mockEventNilMetadata) Metadata() map[string]string { return nil }
func (m *mockEventNilMetadata) Version() string             { return "1.0.0" }
...
func (m *mockEventInvalidTS) Payload() any                { return nil }
